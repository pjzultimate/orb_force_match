...
using namespace std;
// global variables
string first_file = "/home/pjz/hw2/work5/1.png";
string second_file = "/home/pjz/hw2/work5/2.png";
...
int main(int argc, char **argv) {
    ...
    //cv::imwrite("/home/pjz/fff",image_show);
    cv::imwrite("/home/pjz/feat1.png", image_show);
    cv::waitKey(0);
    ...
    //cv::BFMatcher matcher(CV_NORM_HAMMING,true);
    //cv::BFMatcher matche(NORM_HAMMING,true);
   // matcher(descriptors,descriptors2,matches);
   ...
   bfMatch(descriptors, descriptors2, matches);
    ...
    cv::imwrite("/home/pjz/matches.png", image_show);
    cv::waitKey(0);
...
void computeAngle(const cv::Mat &image, vector<cv::KeyPoint> &keypoints) {
    ...
    {
       // if(kp.pt.x-8<0||kp.pt.x+8>image.cols||kp.pt.y-8<0||kp.pt.y+8>image.rows)
      // continue;
    // START YOUR CODE HERE (~7 lines
        ...
        for(int u=-8;u<8;u++)
        {
            for(int v=-8;v<8;v++)
            {
                if(kp.pt.y+v>0&&kp.pt.y+v<image.rows&&kp.pt.x+u>0&&kp.pt.x+u<image.cols)

                {

                m01+=v*image.at<uchar>((kp.pt.y+v),(kp.pt.x+u));
                m10+=u*image.at<uchar>((kp.pt.y+v),(kp.pt.x+u));
            }
                else continue;
                }

        }
        //kp.angle = cv::fastAtan2((float)m01,(float)m10);//atan2(m01,m10)*180/CV_PI;//cv::fastAtan2((float)m01,(float)m10); // compute kp.angle

        kp.angle =atan2(m01,m10)*180/CV_PI;
               if(kp.angle<0)
       {
            kp.angle=kp.angle+360;
...
void computeORBDesc(const cv::Mat &image, vector<cv::KeyPoint> &keypoints, vector<DescType> &desc) {
    for (auto &kp: keypoints)
    {
        ...
        auto u=kp.pt.x;//x int
        auto v=kp.pt.y;//y

        for (int i = 0; i < 256; i++)
           {
        int x=u+ORB_pattern[4*i]*a-ORB_pattern[4*i+1]*b;
        //cout<<"xuan:"<<ORB_pattern[4*i]*a-ORB_pattern[4*i+1]*b<<"hou:"<<cvRound(ORB_pattern[4*i]*a-ORB_pattern[4*i+1]*b)<<endl;
        int y=v+ORB_pattern[4*i]*b+ORB_pattern[4*i+1]*a;
        int x1=u+ORB_pattern[4*i+2]*a-ORB_pattern[4*i+3]*b;
        int y1=v+ORB_pattern[4*i+2]*b+ORB_pattern[4*i+3]*a;
if(x>0&&x<image.cols&&y>0&&y<image.rows&&x1>0&&x1<image.cols&&y1>0&&y1<image.rows)
{           // START YOUR CODE HERE (~7 lines
    if(image.at<uchar>(y,x)<image.at<uchar>(y1,x1)) d[i]=true;
	    // END YOUR CODE HERE  
    }
    else
    {
